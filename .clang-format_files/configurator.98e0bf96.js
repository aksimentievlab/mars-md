...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"../node_modules/file-saver/FileSaver.js":[function(require,module,exports) {
var define;
/* FileSaver.js
...
module.exports = {
  ...
  "useOwnEncryption": false,
  "privKeyPath": "/path/to/privKey.pem",
  "pubKeyPath": "/path/to/pubKey.pem"
};
...
},{"./lib/js-yaml.js":"../node_modules/js-yaml/lib/js-yaml.js"}],"../node_modules/parcel-bundler/src/builtins/_empty.js":[function(require,module,exports) {

},{}],"configurator.js":[function(require,module,exports) {
"use strict";
...
function request_update(clang_options, version) {
  ...
  if (range.start.row != range.end.row && range.start.column != range.end.column) options.range = range.start.row + ':' + range.end.row;
  $.ajax({
    url: _config.default.url + '/format',
    type: 'POST',
    dataType: 'json',
    crossDomain: true,
    success: update_code,
...
function load_config(evt) {
  _.each(evt.target.files, function (file) {
    ...
    reader.onload = function (load_event) {
      ...
      try {
        var data = _jsYaml.default.safeLoad(yml);
      } catch (err) {
...
function save_config(clang_options, version) {
  ...
  if (_.size(config)) {
    try {
      yml = _jsYaml.default.safeDump(config);
    } catch (error) {
      console.log(error);
    }
  } else yml = '';
  ...
  _fileSaver.default.saveAs(blob, '.clang-format');
}
...
function get_config(options, version) {
  ...
  $.each(options[version], function (key, value) {
    ...
    if (option_value && option_value !== 'Default') result[key] = option_value;
  });
  return result;
}
...
function create_input(option_name, option_details) {
  var input_template;
  if (option_details.type === 'BraceWrappingFlags') input_template = flags_input(option_name, option_details.options);else if ($.isArray(option_details.options)) input_template = select_input(option_name, ['Default'].concat(option_details.options));else if (option_details.type === 'bool') input_template = select_input(option_name, ['Default', true, false]);else if (option_details.type === 'std::string' || option_details.type === 'string') input_template = string_input(option_name);else if (option_details.type === 'std::vector<std::string>') input_template = string_input(option_name);else if (option_details.type === 'int') input_template = int_input(option_name);else if (option_details.type === 'unsigned') input_template = int_input(option_name, 0);else {
    console.log('No input created for ' + option_name + ' (type: ' + option_details.type + ')');
  }
  var template = '<div class="form-group">' + '	<label class="col-sm-8">' + '		<a href="#<%= option_name %>_collapse" data-toggle="collapse"><i class="fa fa-info-circle"></i></a>' + '		<%= option_name %>:' + '	</label>' + '	<div class="col-sm-4">' + input_template + '	</div>' + '</div>' + '<div class="collapse" id="<%= option_name %>_collapse">' + '	<div class="well">' + '		<%= option_doc %>' + '	</div>' + '</div>';
...
function select_input(option_name, options) {
  var template = '		<select id="<%= option_name %>" class="form-control">' + '			<% _.forEach(options, function(option){%>' + '				<option value="<%= option %>"><%= option %></option>' + '			<%});%>' + '		</select>';
  return _.template(template)({
    option_name: option_name,
    options: options
  });
...
function string_input(option_name) {
  var template = '<input type="text" class="form-control" id="<%= option_name %>" placeholder="Default"/>';
  return _.template(template)({
    option_name: option_name
  });
}
...
function int_input(option_name, min) {
  var template = '<input type="number" class="form-control" id="<%= option_name %>" placeholder="Default" min="<%= min %>" />';
  return _.template(template)({
    option_name: option_name,
    min: min
  });
...
function flags_input(option_name, options) {
  var template = '<fieldset>' + '<legend><%= option_name %></legend>' + '			<% _.forEach(options, function(option){%>' + '           <input type="checkbox" value="<%= option %>" /><label for="cb1"><%= option %></label><br/>' + '			<%});%>' + '</ul>' + '</fieldset>';
  return _.template(template)({
    option_name: option_name,
    options: options
  });
...
},{"file-saver":"../node_modules/file-saver/FileSaver.js","../config.json":"../config.json","js-yaml":"../node_modules/js-yaml/index.js","fs":"../node_modules/parcel-bundler/src/builtins/_empty.js"}]},{},["configurator.js"], null)
//# sourceMappingURL=/clang-format-configurator/configurator.98e0bf96.js.map