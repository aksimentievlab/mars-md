...
BaseGrid<arbd_real> make_grid(idx_t n, arbd_real dx = arbd_real(1)) {
	BaseGrid<arbd_real> g(Matrix3(dx), Vector3(0, 0, 0), n, n, n);
	g.zero();
	return g;
}

/// Density that is 1 at one voxel and 0 elsewhere.
BaseGrid<arbd_real> delta(idx_t n, idx_t ix, idx_t iy, idx_t iz) {
	auto g = make_grid(n);
	g[lin(g, ix, iy, iz)] = arbd_real(1);
	return g;
}
...
TEST_CASE("Convolution sum rule: integral of output equals product of integrals",
	...
	const arbd_real sum_dens = dens.integrate() / dens.get_cell_volume();
	const arbd_real sum_kern = kern.integrate() / kern.get_cell_volume();
	const arbd_real sum_out = out.integrate() / out.get_cell_volume();
...
TEST_CASE("Volume normalization scales by the density cell volume",
	...
	BaseGrid<arbd_real> dens(Matrix3(arbd_real(2)), Vector3(0, 0, 0), 5, 5, 5);
	dens.zero();
	dens[lin(dens, 2, 2, 2)] = arbd_real(1);
...
TEST_CASE("Convolution preserves the density's geometry", "[grid][convolution]") {
	...
	BaseGrid<arbd_real> dens(Matrix3(arbd_real(1.5)), Vector3(2, 3, 4), 6, 6, 6);
	dens.zero();
