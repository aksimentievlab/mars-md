...
TEST_CASE("DeviceBuffer device-to-device copy", "[device]") {
	...
	DeviceBuffer<float> a(N, res);
	DeviceBuffer<float> b(N, res);
...
TEST_CASE("Zero-size buffers", "[edge]") {
	...
	DeviceBuffer<float> d0(0, res);
	PinnedBuffer<float> p0(0, res);
	UnifiedBuffer<float> u0(0, res);
