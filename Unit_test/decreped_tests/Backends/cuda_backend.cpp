...
TEST_CASE("Manager Basic Initialization", "[Manager][Backend]") {
	SECTION("Initialize and discover devices") {
		REQUIRE_NOTHROW(MARS::CUDA::Manager::init());

		// Check that we found some devices
		const auto& all_devices = MARS::CUDA::Manager::all_devices();
		REQUIRE(!all_devices.empty());
		LOGINFO("Found {} CUDA devices", all_devices.size());

		// Check that at least one device is usable
		REQUIRE_NOTHROW(MARS::CUDA::Manager::load_info());
		const auto& devices = MARS::CUDA::Manager::devices();
...
TEST_CASE("Manager Device Selection and Usage", "[Manager][Backend]") {
	...
	SECTION("Device selection") {
		// Test using device 0
		REQUIRE_NOTHROW(MARS::CUDA::Manager::use(0));
		int current_id = MARS::CUDA::Manager::current();
		REQUIRE((current_id == devices[0].id()));

		// Test current device access
		REQUIRE_NOTHROW(MARS::CUDA::Manager::get_current_device());
		const auto& current_device = MARS::CUDA::Manager::get_current_device();
		REQUIRE((current_device.id() == devices[0].id()));

		// Test cycling through devices if multiple available
		if (devices.size() > 1) {
			REQUIRE_NOTHROW(MARS::CUDA::Manager::use(1));
			current_id = MARS::CUDA::Manager::current();
			REQUIRE((current_id == devices[1].id()));

			// Test wraparound
			int wrapped_id = static_cast<int>(devices.size());
			REQUIRE_NOTHROW(MARS::CUDA::Manager::use(wrapped_id));
			current_id = MARS::CUDA::Manager::current();
	...
	SECTION("Device selection by IDs") {
		...
		REQUIRE_NOTHROW(MARS::CUDA::Manager::load_info());
		const auto& selected_devices = MARS::CUDA::Manager::devices();
...
TEST_CASE("Manager Stream Management", "[Manager][Backend]") {
	MARS::CUDA::Manager::init();
	MARS::CUDA::Manager::load_info();
	MARS::CUDA::Manager::use(0);

	auto& device = MARS::CUDA::Manager::get_current_device();

	SECTION("Stream access") {
		// Test getting specific streams
		for (size_t i = 0; i < MARS::CUDA::Manager::NUM_STREAMS; ++i) {
		...
		for (size_t i = 0; i < MARS::CUDA::Manager::NUM_STREAMS * 2; ++i) {
		...
		REQUIRE(seen_streams.size() == MARS::CUDA::Manager::NUM_STREAMS);
	}
...
TEST_CASE("Manager Synchronization", "[Manager][Backend]") {
	...
	SECTION("Single device sync") {
		REQUIRE_NOTHROW(MARS::CUDA::Manager::sync(0));
	}

	SECTION("All devices sync") {
		REQUIRE_NOTHROW(MARS::CUDA::Manager::sync());
	}
	...
	SECTION("Invalid device sync") {
		...
		REQUIRE_THROWS_AS(MARS::CUDA::Manager::sync(invalid_id), MARS::Exception);
	}
	...
	SECTION("use() with invalid device ID wraps around") {
		...
		REQUIRE_NOTHROW(MARS::CUDA::Manager::use(invalid_id));
		int current_id = MARS::CUDA::Manager::current();
		REQUIRE((current_id == devices[0].id())); // Should wrap to 0
	}
}
...
TEST_CASE("Manager Safe Device Handling", "[Manager][Backend]") {
	...
	const auto& all_devices = MARS::CUDA::Manager::all_devices();
	const auto& safe_devices = MARS::CUDA::Manager::safe_devices();
	...
	SECTION("Prefer safe devices") {
		...
		MARS::CUDA::Manager::prefer_safe_devices(true);
		MARS::CUDA::Manager::load_info();
		...
		MARS::CUDA::Manager::prefer_safe_devices(false);
		MARS::CUDA::Manager::load_info();

		const auto& all_loaded_devices = MARS::CUDA::Manager::devices();
...
TEST_CASE("Manager Peer Access", "[Manager][Backend]") {
	...
	SECTION("Peer access matrix") {
		if (devices.size() > 1) {
			const auto& peer_matrix = MARS::CUDA::Manager::peer_access_matrix();
...
TEST_CASE("Manager Finalization", "[Manager][Backend]") {
	...
	SECTION("Clean finalization") {
		REQUIRE_NOTHROW(MARS::CUDA::Manager::finalize());
