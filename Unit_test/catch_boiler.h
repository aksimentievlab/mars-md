...
namespace Global {
static short single_resource_id = 7; // For testing, we use a single resource id
  ...
  public:
	...
	void initialize() {
		...
		try {
			...
			if (rank_ == 0) {
				std::cout << "MPI Test Manager initialized with " << size_ << " processes"
						  << std::endl;
			}
		} catch (const std::exception& e) {
			std::cerr << "MPI Test Manager initialization failed: " << e.what() << std::endl;
		}
	}
	...
	bool runCoordinatedTest(TestFunc test_func, VerifyFunc root_verify_func) {
		...
		try {
			...
			if (rank_ == 0) {
				std::cerr << "Test function failed on rank " << rank_ << ": " << e.what()
						  << std::endl;
			}
			local_success = false;
		}
	...
	bool runCoordinatedTest(TestFunc test_func) {
