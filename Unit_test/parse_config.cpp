...
void test_parse_circovirus_config() {
	...
	try {
		...
		const Configuration& config = config_parser.get_config();
		...
		std::cout << "Box size: " << config.sim_box.get_box_size().x << " x "
				  << config.sim_box.get_box_size().y << " x " << config.sim_box.get_box_size().z
				  << " Å" << std::endl;
		...
		assert(config.sim_box.get_box_size().x == 5000.0f);
		assert(config.sim_box.get_box_size().y == 5000.0f);
		assert(config.sim_box.get_box_size().z == 5000.0f);
		...
		const auto& particle_types = config.particle_types;
		...
		const auto& particles = config.init_particles;
		...
		const auto& bonds = config.init_bonds;
		const auto& angles = config.init_angles;
		const auto& dihedrals = config.init_dihedrals;
		const auto& exclusions = config.init_exclusions;
...
inputAngles nonexistent_angles.txt
	...
	try {
		ConfigParser config_parser(test_file);
		const Configuration& config = config_parser.get_config();

		std::cout << "✓ Configuration loaded successfully with missing files" << std::endl;
		std::cout << "Bonds loaded: " << config.init_bonds.size() << std::endl;
		std::cout << "Angles loaded: " << config.init_angles.size() << std::endl;

		// Should handle missing files gracefully (empty lists)
		assert(config.init_bonds.empty());
		assert(config.init_angles.empty());
...
inputAngles ../test_angles.txt
	...
	try {
		...
		assert(config.init_bonds.size() == 2);
		assert(config.init_angles.size() == 1);
...
void test_config_validation() {
	...
	try {
		ConfigParser config_parser(invalid_config);
	...
	try {
		ConfigParser config_parser(valid_config);
