...
class MPIBackend {
  public:
	...
	static void* allocate(size_t size, const MPIResource& resource) {
		...
		if (!ptr) {
			ARBD::throw_value_error("Host memory allocation failed on rank {}", resource.rank);
		}
	 ...
	 */
	template<typename T>
	static void send_data(const T* data, size_t count, int dest_rank, int tag = 0) {
#ifdef USE_MPI
...
#ifdef USE_MPI
		MPI_Status status;
		MPI_Recv(data, count * sizeof(T), MPI_BYTE, src_rank, tag, MPI_COMM_WORLD, &status);
	 ...
	 */
	template<typename T, typename Func, typename... Args>
	static std::future<T> execute_on_rank_async(int target_rank, Func&& func, Args&&... args) {
