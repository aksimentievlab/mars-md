 ...
 */
enum class GridBoundaryCondition : int {
 ...
 *          and RigidBodyTypeView's grid ranges). Deliberately a 16-byte POD: a
 *          grid-force kernel reads one of these per grid in its inner loop, and
 *          a single load beats four parallel-array loads.
 *
