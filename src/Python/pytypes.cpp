...
///
/// This function converts a 2D NumPy array of shape [N,M] with M in (3,4) into an
/// Array<Vector3_t<T> object.
///
/// \tparam T - The data type of the elements in the NumPy array.
/// \param a - The NumPy array to convert.
...
///
/// This function converts an Array<Vector3_t<T> object into a 2D NumPy array of shape [N,M] with M
/// in (3,4).
///
...
auto vector_arr_to_numpy_array(Array<Vector3_t<T>>& inp) {
	...
	// Vector3_t<T> tmp(1);
	// assert( reinterpret_cast<unsigned char*>(&(tmp.x)) - reinterpret_cast<unsigned char*>(&(tmp))
	// == 0 ); cptr += reinterpret_cast<unsigned char*>(&(tmp.x)) - reinterpret_cast<unsigned
	// char*>(&(tmp)); T* ptr = reinterpret_cast<T*>(cptr);
	...
	std::vector<ssize_t> strides = {static_cast<ssize_t>(sizeof(Vector3_t<T>)),
									static_cast<ssize_t>(sizeof(T))};

	assert(sizeof(Vector3_t<T>) == 4 * sizeof(T));
	auto a = py::template array_t<T>(shape,			  // shape
									 strides,		  // strides
									 ptr,			  // data pointer
