#pragma once
/**
 * @brief nanobind type_casters converting MARS::Vector3_t<T>/Matrix3_t<T>
...
template<typename T>
struct type_caster<MARS::Vector3_t<T>> {
	using Array = nanobind::ndarray<T, nanobind::numpy, nanobind::shape<3>>;

	NB_TYPE_CASTER(MARS::Vector3_t<T>, const_name("Vector3"))
	...
	bool from_python(handle src, uint32_t /*flags*/, cleanup_list*) noexcept {
		...
		try {
			if (len(src) != 3)
				return false;
			value = MARS::Vector3_t<T>(cast<T>(src[0]), cast<T>(src[1]), cast<T>(src[2]));
			return true;
		} catch (...) {
			PyErr_Clear();
			return false;
	...
	// C++ -> Python
	static handle from_cpp(const MARS::Vector3_t<T>& src, rv_policy, cleanup_list*) noexcept {
 ...
 */
template<typename T>
struct type_caster<MARS::Matrix3_t<T>> {
	using Array = nanobind::ndarray<T, nanobind::numpy, nanobind::shape<3, 3>>;

	NB_TYPE_CASTER(MARS::Matrix3_t<T>, const_name("Matrix3"))
	...
	bool from_python(handle src, uint32_t /*flags*/, cleanup_list*) noexcept {
		...
		try {
			...
			value = MARS::Matrix3_t<T>(MARS::Vector3_t<T>(r[0][0], r[1][0], r[2][0]),
									   MARS::Vector3_t<T>(r[0][1], r[1][1], r[2][1]),
									   MARS::Vector3_t<T>(r[0][2], r[1][2], r[2][2]));
			return true;
		} catch (...) {
			PyErr_Clear();
			return false;
	...
	static handle from_cpp(const MARS::Matrix3_t<T>& src, rv_policy, cleanup_list*) noexcept {
