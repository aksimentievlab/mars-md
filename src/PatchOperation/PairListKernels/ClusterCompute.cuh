...
struct BoundBox {
  public:
	// constructors
	__host__ __device__ BoundBox(Vector3* points, int num) {
		if (points == NULL || num < 1)
			return;
	...
	__device__ BoundBox(int* parts, int num, cudaTextureObject_t PosTex) {
		if (parts == NULL || num < 1)
			return;
	...
	within(const float dist2, const BoundBox& b, const float3& bound) const {
		...
		// #ifdef DEBUG
		// if ((x1-x0 < 0.25*bound.x) || (y1-y0 < 0.25*bound.y) || (b.x1-b.x0 < 0.25*bound.x) ||
		// (b.y1-b.y0 < 0.25*bound.y)) {
		...
		if (dx2 > dist2 && dx2 < 0.25 * bound.x * bound.x)
			return false;
		int dy2 = (y1 - b.y0) * (y1 - b.y0);
		if (dy2 > dist2 && dy2 < 0.25 * bound.y * bound.y)
			return false;
	 ...
	 *          bound: boundary of simulation box
	 */
	__host__ __device__ float dist2(const BoundBox& other, const float3& bound) const {
		// note: assume two bounding boxes do not intersect or one contains the other,
		// because bounding boxes are constructed on exclusive clusters
		// reference:
		// https://gamedev.stackexchange.com/questions/154036/efficient-minimum-distance-between-two-axis-aligned-squares
		...
		float b0_x0 = x0, b0_x1 = x1, b0_y0 = y0, b0_y1 = y1, b0_z0 = z0, b0_z1 = z1,
			  b1_x0 = other.x0, b1_x1 = other.x1, b1_y0 = other.y0, b1_y1 = other.y1,
	...
	pbc_dist(const float& a, const float& b, const float& bound) const {
...
__global__ void build_pencils_kernel(int* pencil_particle_num,
 ...
 * @params:    clusters_d: caller-allocated sufficiently large Cluster array to store constructed
 * clusters [output] bb_d: caller-allocated sufficiently large BoundBox array to store bounding
 * boxes of clusters [output] cluster_num_d: a int pointer to somewhere to store the number of
 * clusters constructed[output] grid_particle_num: number of particles for each cluster gird
 *    grid_particle_array: a pointer to an array of allocated space to temporarily store particles
 * for sorting and clustering pos_arr_d: position of all particles len: number of particles
 ...
 *
 * NOTE: 1. number of clusters is slightly larger than len/cluster_size, allocate 2 *
 * (len/cluster_size) elements for clusters_d and bb_d for safety
 *       2. build_clusters_kernel should be called immediately after particle_histogram_kernel
 */
__global__ void build_clusters_kernel(int cluster_size,
									  int2* clusters_d,
									  BoundBox* bb_d,
									  int* cluster_num,
									  int num_pencils,
									  int* pencil_particle_num,
									  int* pencil_offset,
									  int* pencil_particle_idx,
									  cudaTextureObject_t PosTex) {
	...
	if (pencil_idx < num_pencils) {
		...
		for (int start_idx = tid * cluster_size; start_idx < particle_num;
			 start_idx += cluster_size * blockDim.x) {
			int part_num = particle_num - start_idx < cluster_size
							   ? particle_num - start_idx
							   : cluster_size; // particles in cluster
...
__global__ void calc_cluster_zorder_idx(int2* clusters_d,
										BoundBox* bb_d,
										int* cluster_num,
										float cutoff,
	...
	Vector3 o;
	{
		...
		dx = B.ex().x;
		dy = B.ey().y;
		dz = B.ez().z; // Only for rectilinear unit cells
...
// __global__ void reorder_particles(int cluster_size, int2* clusters_d, BoundBox* bb_d, int
// *cluster_num, int num_pencils, int *pencil_particle_num, int* pencil_offset, int*
// pencil_particle_idx, cudaTextureObject_t PosTex) {
...
// 	    for(int start_idx = tid*cluster_size; start_idx < particle_num; start_idx +=
// cluster_size*blockDim.x) { 		int part_num = particle_num-start_idx < cluster_size ?
// particle_num-start_idx : cluster_size; // particles in cluster 		int cluster_idx =
// atomicAdd(cluster_num, 1); 		clusters_d[cluster_idx] = make_int2(pidx_start+start_idx,
// part_num); 		BoundBox& bb = bb_d[cluster_idx] = BoundBox(); 		for (int j = 0; j < part_num; ++j) {
// 		    const int ai = pencil_particle_idx[ pidx_start+start_idx+j ];
// 		    Vector3 pos = Vector3(tex1Dfetch<float4>(PosTex, ai));
// 		    bb.add_point( pos );
// 		}
...
__global__ void sort_particles_by_pencil_z(int particle_num,
										   int* __restrict__ pencil_particle_idx,
										   cudaTextureObject_t PosTex,
 ...
 *        neighbor_list: caller-allocated sufficiently large int2 array to store neighbor cluster
 * indices [output] neighbor_num_d: a int pointer to somewhere to store the number of neighbor
 * cluster pair [output] cutoff2: square of cutoff distance bound: boundary of simulation box
 *
 * NOTE: 1. the maximum length of neighbor_list can be 0.5*(cluster_num)^2, allocate that number for
 * neighbor_list for safety
 *       2. neighbor_list does not allow repetition(e.g. (1,2) but no (2,1)), but allow
 * self-pair(e.g. (2,2))
 */
__global__ void build_cluster_pair_list_kernel_A(const int cluster_num,
												 int2* neighbor_list,
												 int* neighbor_num_d,
												 const BaseGrid* __restrict__ sys,
												 BoundBox* bb_d,
...
// __global__ void build_cluster_pair_list_kernel_B(const int cluster_num, int2 *neighbor_list, int
// *neighbor_num_d) {
//     int tid = threadIdx.x;
...
__global__ void
calculate_force_clusters_build_excl_kernel(Vector3* force,
										   const BaseGrid* __restrict__ sys,
										   const float cutoff2,
										   const float pairlist_dist2,
										   const int num_cluster_pairs,
										   int2* neighbor_list,
 ...
 *         neighbor_list_compact: shortened neighbor cluster indices array without dummy pairs
 * [output] neighbor_check: an array of counters of number of particles in cluster i that are
 * neighbor of any particles in cluster j for pair(i,j), same length as neighbor_list
 ...
 *
 * NOTE: calculate_force_clusters_kernel is called after calculate_force_clusters_kernel_trial is
 * already called once
 */
__global__ void
calculate_force_clusters_kernel(Vector3* force,
								const BaseGrid* __restrict__ sys,
								const float cutoff2,
								const int num_cluster_pairs,
								int2* __restrict__ neighbor_list,
								const int num_clusters,
								int2* __restrict__ clusters,
...
// Vector3* __restrict__ pos)
{
	...
	Vector3* __restrict__ force_j =
		(Vector3*)&particle_j[(2 + sizeof(Vector3) / sizeof(int)) * block_size];
	...
	while (pair_id < num_cluster_pairs) {
		...
		if (ai >= 0) {
			...
			if (pair_id >= num_cluster_pairs) {
				...
				for (int j = 0; j < cluster_j.y; ++j) {
					// const size_t ex_idx = ((size_t) pair_id)*cluster_size*cluster_size +
					// j*cluster_size + i; if (mask->get_mask( ex_idx )) { 	continue;
					// }
				...
				for (int j = 0; j < cluster_j.y; ++j) {
					const size_t ex_idx =
						((size_t)pair_id) * cluster_size * cluster_size + j * cluster_size + i;
