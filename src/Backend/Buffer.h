...
class Buffer {
  ...
  public:
	 ...
	 */
	Buffer() = default;
	explicit Buffer(size_t count) : count_(count), resource_(get_device_resource(0)) {
		if (count_ > 0) {
			stream_ = resource_.get_stream(stream_type_); // Use Memory stream by default
	...
	explicit Buffer(size_t count, short device_id, void* queue, bool sync = false)
		: count_(count), resource_(get_device_resource(device_id)), stream_(queue), sync_(sync) {
		if (count_ > 0) {
			allocate_on_resource(resource_, count_, stream_, sync_);
		}
	}
	...
	Buffer(Buffer&& other) noexcept
		: resource_(other.resource_), count_(other.count_), device_ptr_(other.device_ptr_),
		  stream_(other.stream_), sync_(other.sync_) {
		other.count_ = 0;
...
#endif
	static Buffer create(size_t count, int device_id, const Buffer* pool = nullptr) {
		Buffer new_buffer;
		new_buffer.resource_ = pool ? pool->resource() : get_device_resource(device_id);

		// Allocate memory directly
		new_buffer.device_ptr_ = static_cast<T*>(
			Policy::allocate(new_buffer.resource_, count * sizeof(T), nullptr, true));
