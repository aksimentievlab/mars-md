...
#ifdef USE_CUDA
	if (resource.type() == ResourceType::CUDA) {
		return launch_cuda_kernel_with_workitem(resource,
												local_config,
												kernel_functor,
												get_buffer_pointer(std::forward<Args>(args))...);
	}
...
class KernelPipeline {
  ...
  public:
	...
	template<typename Functor, typename... Args>
	KernelPipeline& then(idx_t thread_count,
						 Functor kernel_functor,
						 const KernelConfig& base_config,
						 Args... args) {
		...
		Event completion = launch_kernel(resource_, config, kernel_functor, args...);
