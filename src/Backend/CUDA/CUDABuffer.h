...
struct Policy {
	...
	fill(void* dst, T value, size_t num_elements, void* queue = nullptr, bool sync = false) {
		...
		if (value == T{}) { // T{} is 0 for numbers, false for bool, etc.
			...
			if (sync) {
				...
				CUDA_CHECK(cudaMemsetAsync(dst, 0, bytes, stream));
			}
		} else {
			// GENERAL CASE: Use the kernel for non-zero values
			const int block_size = 256;
			const int num_blocks = (num_elements + block_size - 1) / block_size;

			fill_kernel<T>
				<<<num_blocks, block_size, 0, stream>>>(static_cast<T*>(dst), value, num_elements);
	...
	struct PinnedPolicy {
		...
		static void upload_to_device(void* device_dst,
			...
			CUDA_CHECK(
				cudaMemcpyAsync(device_dst, pinned_src, bytes, cudaMemcpyHostToDevice, stream));
		}
		...
		static void download_from_device(void* pinned_dst,
			...
			CUDA_CHECK(
				cudaMemcpyAsync(pinned_dst, device_src, bytes, cudaMemcpyDeviceToHost, stream));
		}
	};
	...
	struct UnifiedPolicy {
		...
		static void copy_to_host(void* host_dst,
			...
			// Prefetch to host
			CUDA_CHECK(cudaMemPrefetchAsync(const_cast<void*>(unified_src),
											bytes,
											cudaCpuDeviceId,
											stream));
			if (stream) {
	...
	struct TexturePolicy {
		...
		static void* allocate(const Resource& resource,
			...
			CUDA_CHECK(
				cudaCreateTextureObject(&texture->textureObject, &resDesc, &texDesc, nullptr));
