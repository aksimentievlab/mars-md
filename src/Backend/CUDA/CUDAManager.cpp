...
void Manager::init() {
	...
	initialized_ = true;
	LOGINFO("CUDA Manager initialized with {} device(s)", device_properties_.size());
}
...
void Manager::discover_devices() {
	...
	for (int dev = 0; dev < num_devices; ++dev) {
		...
		LOGINFO("Found CUDA device [{}]: {} (SM {}.{}, {:.2f} GB)",
				dev,
				props.name,
				props.major,
				props.minor,
	...
	for (size_t i = 0; i < num_devices; ++i) {
		...
		for (size_t j = i + 1; j < num_devices; ++j) {
			...
			if (can_access_ij || can_access_ji) {
				LOGDEBUG("P2P access between devices {} and {}: {} <-> {}",
						 i,
						 j,
						 can_access_ij ? "yes" : "no",
						 can_access_ji ? "yes" : "no");
			}
		}
	}
}
...
bool Manager::can_access_peer(int device1, int device2) {
	...
	if (device1 < 0 || device2 < 0 || device1 >= static_cast<int>(peer_access_matrix_.size()) ||
...
#pragma omp single
	...
	if (ranks_per_node == 1) {
		...
		LOGINFO("Rank {} assigned to {} GPU(s): [{}]", local_rank, rank_devices_.size(), [&]() {
			std::stringstream ss;
	...
	if (false && rank_devices_.size() > 1) {
		for (size_t i = 0; i < rank_devices_.size(); ++i) {
			for (size_t j = i + 1; j < rank_devices_.size(); ++j) {
				int can_access = 0;
				CUDA_CHECK(
					cudaDeviceCanAccessPeer(&can_access, rank_devices_[i], rank_devices_[j]));
				if (can_access) {
					...
					if (err == cudaSuccess || err == cudaErrorPeerAccessAlreadyEnabled) {
						...
						LOGDEBUG("Peer access enabled: GPU {} <-> GPU {}",
								 rank_devices_[i],
								 rank_devices_[j]);
					} else {
...
void Manager::set_omp_gpu_affinity(const std::string& strategy) {
	...
	if (strategy != "block" && strategy != "cyclic") {
		LOGWARN("Invalid GPU affinity strategy . Use 'block' or 'cyclic'", strategy);
		return;
	}
