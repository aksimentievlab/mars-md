...
class InitStreams {
	...
	cudaStream_t get_next_stream() {
		if (!initialized_) {
			MARS_Exception(ExceptionType::RuntimeError,
						   "InitStreams not initialized for device {}",
						   device_id_);
		}
  ...
  private:
	...
	InitStreams(InitStreams&& other) noexcept
		: streams_(std::move(other.streams_)), next_index_(other.next_index_.load()),
		  device_id_(other.device_id_), initialized_(other.initialized_) {
