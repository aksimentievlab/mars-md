...
struct TexturePolicy {
	// Stores a single non-templated sycl::image pointer (1D/2D/3D created based on dims)
	struct Texture {
	...
	static void copy_from_buffer(void* texture_ptr,
		...
		sycl::buffer<sycl::float4, 1> src_buf(reinterpret_cast<const sycl::float4*>(buffer_ptr),
											  sycl::range<1>(num_pixels),
		...
		q.submit([&](sycl::handler& cgh) {
			 ...
			 if (is3D) {
				 ...
				 cgh.parallel_for(sycl::range<3>(tex->width, tex->height, tex->depth),
								  [=](sycl::id<3> idx) {
				 ...
				 cgh.parallel_for(sycl::range<2>(tex->width, tex->height), [=](sycl::id<2> idx) {
	...
	/**
		static void copy_from_buffer2(void* texture_ptr,
			...
			sycl::buffer<unsigned char, 1> src_buf(static_cast<const unsigned char*>(buffer_ptr),
												   sycl::range<1>(bytes),
												   {sycl::property::buffer::use_host_ptr()});
			...
			q.submit([&](sycl::handler& cgh) {
				 ...
				 if (tex->depth > 0) {
					 auto* img = static_cast<sycl::image<3>*>(tex->image);
					 ...
					 // Use parallel_for to copy data from buffer to image
					 cgh.parallel_for(sycl::range<1>(num_elements), [=](sycl::id<1> idx) {
						 if (idx[0] < num_elements) {
							 // Convert 1D index to 3D coordinates if needed
							 // For now, assume linear mapping
							 const unsigned char* src_ptr = &src_acc[idx * sizeof(sycl::float4)];
							 sycl::float4 data = *reinterpret_cast<const sycl::float4*>(src_ptr);
							 sycl::id<3> coord(idx[0] % tex->width,
											   (idx[0] / tex->width) % tex->height,
											   idx[0] / (tex->width * tex->height));
							 dest_acc[coord] = data;
						 }
					 });
				 } else if (tex->height > 0) {
					 auto* img = static_cast<sycl::image<2>*>(tex->image);
					 ...
					 cgh.parallel_for(sycl::range<1>(num_elements), [=](sycl::id<1> idx) {
						 if (idx[0] < num_elements) {
							 const unsigned char* src_ptr = &src_acc[idx * sizeof(sycl::float4)];
							 sycl::float4 data = *reinterpret_cast<const sycl::float4*>(src_ptr);
							 sycl::id<2> coord(idx[0] % tex->width, idx[0] / tex->width);
							 dest_acc[coord] = data;
						 }
					 });
				 } else {
					 auto* img = static_cast<sycl::image<1>*>(tex->image);
