...
struct WorkItem {
	...
	DEVICE WorkItem(ItemType item, LocalAccessorType local_acc, size_t shared_mem_size)
	...
	template<typename T>
	DEVICE T* get_shared_mem(size_t offset_bytes = 0) {
		// Get pointer to the underlying data - no casting if types match
		auto multi_ptr = local_acc_.template get_multi_ptr<sycl::access::decorated::no>();
		auto base_ptr = multi_ptr.get();

		// If T matches the accessor type, return directly; otherwise cast
		if constexpr (std::is_same_v<T, typename std::remove_reference_t<decltype(*base_ptr)>>) {
...
Event launch_sycl_kernel_with_workitem(const Resource& resource,
	...
	auto sycl_event = queue.submit([&](sycl::handler& h) {
		...
		size_t num_elements =
			(config.shared_memory > 0 ? config.shared_memory : sizeof(memT)) / sizeof(memT);
		...
		h.parallel_for(execution_range,
					   [kernel_functor, shared_mem_size, smem_acc, args...](sycl::nd_item<1> item) {
