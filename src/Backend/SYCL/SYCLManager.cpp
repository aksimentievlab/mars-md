...
void Manager::init() {
	...
	if (all_devices_.empty()) {
		MARS_Exception(ExceptionType::ValueError, "No SYCL devices found");
	}
...
void Manager::load_info() {
	...
	for (size_t i = 0; i < all_devices_.size(); ++i) {
		try {
			...
			LOGDEBUG("  Global memory: {:.2f} GB", global_mem / (1024.0 * 1024.0 * 1024.0));
			LOGDEBUG("  Local memory: {:.2f} KB", local_mem / 1024.0);

		} catch (const sycl::exception& e) {
			LOGWARN("Could not query detailed device info for device {}: {}", i, e.what());
		}
	}
}
...
sycl::device Manager::get_device_by_id(size_t device_id) {
	...
	if (device_id >= all_devices_.size()) {
		MARS_Exception(ExceptionType::ValueError,
					   "SYCL device {} not found (available: 0-{})",
					   device_id,
					   all_devices_.size() - 1);
	}
...
void Manager::init_for_rank(int local_rank,
							int ranks_per_node,
...
#pragma omp single
	...
	if (num_devices == 0) {
		MARS_Exception(ExceptionType::ValueError,
					   "No SYCL devices available for rank {}",
					   local_rank);
	}
	...
	if (ranks_per_node == 1) {
		...
		LOGINFO("Rank {} assigned to {} SYCL device(s): [{}]",
				local_rank,
				rank_devices_.size(),
				[&]() {
					std::stringstream ss;
...
#pragma omp parallel
		{
			...
			if (thread_id < static_cast<int>(thread_device_map_.size())) {
				...
				LOGTRACE("OpenMP thread {} assigned to SYCL device {}", thread_id, assigned_device);
			}
		}
#endif

		LOGINFO("OpenMP configuration complete: {} threads across {} SYCL device(s)",
				omp_threads_,
				rank_devices_.size());
	}

	LOGINFO("SYCL Manager initialized for rank {} with {} device(s) and {} OpenMP "
			"thread(s)",
			local_rank,
			rank_devices_.size(),
			omp_threads_);
...
void Manager::setup_omp_device_mapping() {
	...
	if (device_affinity_strategy_ == "block") {
		...
		int threads_per_device = (omp_threads_ + num_rank_devices - 1) / num_rank_devices;
		...
		// Default to block
		LOGWARN("Unknown device affinity strategy '{}', using 'block'", device_affinity_strategy_);
...
void Manager::set_omp_device_affinity(const std::string& strategy) {
	...
	if (strategy != "block" && strategy != "cyclic") {
		LOGWARN("Invalid device affinity strategy '{}'. Use 'block' or 'cyclic'", strategy);
		return;
	}
