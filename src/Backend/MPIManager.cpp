...
void Manager::init(bool assume_gpu_aware) {
	...
	MPI_CHECK(MPI_Init_thread(nullptr, nullptr, MPI_THREAD_MULTIPLE, &provided_thread_level));

	if (provided_thread_level < MPI_THREAD_MULTIPLE) {
		LOGWARN("MPI does not support MPI_THREAD_MULTIPLE, got level: {}", provided_thread_level);
	}
	...
	LOGINFO("MPI Manager initialized: rank {}/{} (GPU-aware: {})",
			rank_,
			size_,
...
CommunicationProtocol Manager::get_communication_protocol(const Resource& resource) const {
...
void Manager::wait_all() {
	...
	for (const auto& op : pending_operations_) {
		auto duration =
			std::chrono::duration_cast<std::chrono::microseconds>(now - op.start_time).count();
...
void Manager::detect_gpu_aware_capabilities(std::optional<MPIAwareState> forced_state) {
	...
	try {
		...
		if (size_ >= 2) {
			...
			if (protocol == CommunicationProtocol::DirectGPU) {
				...
				MPI_CHECK(MPI_Isend(send_buffer.data(),
									test_size,
									MPI_FLOAT,
									next_rank,
									0,
									comm_,
									&send_req));
				MPI_CHECK(MPI_Irecv(recv_buffer.data(),
									test_size,
									MPI_FLOAT,
									prev_rank,
									0,
				...
				MPI_CHECK(MPI_Isend(send_host.data(),
									test_size,
									MPI_FLOAT,
									next_rank,
									0,
									comm_,
									&send_req));
				MPI_CHECK(MPI_Irecv(recv_host.data(),
									test_size,
									MPI_FLOAT,
									prev_rank,
									0,
...
void Manager::print_info() const {
	if (rank_ == 0) {
		...
		const char* cuda_state_str = (cuda_aware_state_ == MPIAwareState::Yes)	? "Yes"
									 : (cuda_aware_state_ == MPIAwareState::No) ? "No"
									 : (cuda_aware_state_ == MPIAwareState::ForcedYes)
										 ? "Forced Yes"
		...
		const char* rocm_state_str = (rocm_aware_state_ == MPIAwareState::Yes)	? "Yes"
									 : (rocm_aware_state_ == MPIAwareState::No) ? "No"
									 : (rocm_aware_state_ == MPIAwareState::ForcedYes)
										 ? "Forced Yes"
		...
		LOGINFO("  - CUDA/SYCL: {}",
				is_direct_comm_capable(ResourceType::SYCL) ? "DirectGPU" : "CopyToHost");
