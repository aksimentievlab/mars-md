...
#ifndef PI
// TODO: move to some commmon header, consolidate with similar defs
...
Configuration::Configuration(const char* config_file, int simNum, bool debug) : simNum(simNum) {
	// Read the parameters.
	// type_d = NULL;
	kTGrid_d = NULL;
	// bonds_d = NULL;
	...
	if (numCap <= 0)
		numCap = numCapFactor * num; // max number of particles
	if (numCap <= 0)
		numCap = 20;
	...
	if (ParticleDynamicType == String("Langevin") ||
		ParticleDynamicType == String("NoseHooverLangevin"))
	...
	if (restartCoordinates.length() > 0) {
		...
		if (ParticleDynamicType == String("Langevin") ||
			...
			if (restartMomentum.length() > 0) {
				...
				printf("Warning: There is no restart momentum file when using restart coordinates "
		...
		if (numPartsFromFile > 0) {
			...
			for (int i = 0; i < num; i++) {
				...
				if (ParticleDynamicType == String("Langevin") ||
					...
					loadedMomentum = true;
					if (numTokens == 9)
						momentum[i] = Vector3(atof(tokenList[6].val()),
											  atof(tokenList[7].val()),
											  atof(tokenList[8].val()));
					else {
	...
	if (readBondAnglesFromFile)
		readBondAngles();
	if (readProductPotentialsFromFile)
		readProductPotentials();
	...
	for (int i = 0; i < numParts; i++) {
		for (int j = 0; j < part[i].numPartGridFiles; ++j) {
			...
			if (part_grid_dictionary.count(fname) == 0) {
				...
				if (len >= 3 && fname[len - 3] == '.' && fname[len - 2] == 'd' &&
					fname[len - 1] == 'x') {
					part_grid_dictionary.insert({fname, BaseGrid(fname.c_str())});
				} else if (len >= 4 && fname[len - 4] == '.' && fname[len - 3] == 'd' &&
						   fname[len - 2] == 'e' && fname[len - 1] == 'f') {
					assert(1 ==
						   2); // Throw exception because this implementation needs to be revisited
							   /*                OverlordGrid* over = new
								  ...
								  ++j)
										 ...
										 len = map.length();
										 if (!(len >= 4 && map[len-4]=='.' && map[len-3]=='d' &&
								  map[len-2]=='e' && map[len-1]=='f'))
										 {
	...
	for (int i = 0; i < numParts; i++) {
		...
		part[i].pmf_scale_slope = new float[part[i].numPartGridFiles];
		part[i].pmf_smd_freq = new float[part[i].numPartGridFiles];
		...
		for (int j = 0; j < part[i].numPartGridFiles; ++j) {
			...
			part[i].pmf_scale_slope[j] = partGridFileScaleSlope[i][j];
			part[i].pmf_smd_freq[j] = partGridFileSMD[i][j];
			part[i].meanPmf[j] =
				grid_mean_dict.find(std::string(partGridFile[i][j]))->second * part[i].pmf_scale[j];
		}
		if (partForceXGridFile[i].length() != 0) {
			part[i].forceXGrid = new BaseGrid(partForceXGridFile[i].val());
			if (partForceGridScale[i] != nullptr)
				part[i].forceXGrid->scale(partForceGridScale[i][0]);
		...
		if (partForceYGridFile[i].length() != 0) {
			...
			if (partForceGridScale[i] != nullptr)
				part[i].forceYGrid->scale(partForceGridScale[i][1]);
	...
	if (excludeRule != String("")) {
		...
		Exclude* newExcludes =
			makeExcludes(bonds, bondMap, num, numBonds, excludeRule, numExcludes);
		if (excludes == NULL) {
	...
	if (restartCoordinates.length() > 0) {
		...
		if (numPartsFromFile > 0) {
			for (int i = 0; i < num; i++) {
				...
				pos[i] = Vector3(atof(tokenList[3].val()), atof(tokenList[4].val()),
	atof(tokenList[5].val()));
			}
...
Configuration::~Configuration() {
	...
	for (int i = 0; i < numParts; ++i) {
		...
		if (partGridFileScaleSlope[i] != NULL) {
			delete[] partGridFileScaleSlope[i];
			partGridFileScaleSlope[i] = NULL;
		}
	}
	delete[] partGridFile;
	delete[] partGridFileScale;
	delete[] partGridFileScaleSlope;
	...
	if (bondAngles != NULL)
		delete[] bondAngles;
	if (productPotentials != NULL)
		delete[] productPotentials;
...
void Configuration::copyToCUDA() {
	...
	for (const auto& pair : part_grid_dictionary) {
		...
		// printf("Assigning grid for %s to %p (originally %p)\n", pair.first.c_str(), (void *)
		// part_grid_dictionary_d[pair.first], (void *) g_d);
	}
	...
	for (int i = 0; i < numParts; i++) {
		...
		if (part[i].pmf != NULL) {
			{
				...
				for (int j = 0; j < part[i].numPartGridFiles; ++j) {
					// printf("Retrieving grid for %s (at %p)\n", partGridFile[i][j].val(), (void *)
					// part_grid_dictionary_d[std::string(partGridFile[i][j])]);
				...
				gpuErrchk(cudaMemcpy(tmp_d,
									 tmp,
									 sizeof(BaseGrid*) * part[i].numPartGridFiles,
...

			{
				...
				gpuErrchk(cudaMemcpy(tmp,
									 part[i].pmf_scale,
									 sizeof(float) * part[i].numPartGridFiles,
									 cudaMemcpyHostToDevice));
				b->pmf_scale = tmp;
			}
			{
				...
				gpuErrchk(cudaMalloc(&tmp, sizeof(float) * part[i].numPartGridFiles));
				gpuErrchk(cudaMemcpy(tmp,
									 part[i].pmf_scale_slope,
									 sizeof(float) * part[i].numPartGridFiles,
									 cudaMemcpyHostToDevice));
				b->pmf_scale_slope = tmp;
			}
...

			{
				...
				gpuErrchk(
					cudaMemcpy(tmp, part[i].pmf_boundary_conditions, s, cudaMemcpyHostToDevice));
		...
		gpuErrchk(
			cudaMemcpyAsync(part_addr[i], b, sizeof(BrownianParticleType), cudaMemcpyHostToDevice));
	}
	// RBTODO: moved this out of preceding loop; was that correct?
	gpuErrchk(cudaMemcpyAsync(part_d,
							  part_addr,
							  sizeof(BrownianParticleType*) * numParts,
							  cudaMemcpyHostToDevice));
	...
	gpuErrchk(cudaMemcpyAsync(type_d, type, sizeof(int+num_rb_attached_particles) * num * simNum,
	cudaMemcpyHostToDevice));
...
int Configuration::readParameters(const char* config_file) {
	...
	// partGridFileScale = new float[numParts];
	partGridFileScale = new float*[numParts];
	partGridFileScaleSlope = new float*[numParts];
	partGridFileSMD = new float*[numParts];
	...
	for (int i = 0; i < numParts; ++i) {
		partGridFile[i] = NULL;
		partGridFileScale[i] = NULL;
		partGridFileScaleSlope[i] = nullptr;
	...
	for (int i = 0; i < numParams; i++) {
		...
		// PARTICLES
			...
			if (tmp != 3) {
				printf(
					"ERROR: Expected three floating point scale values for x,y,z, but got `%s'.\n",
					param.val());
				exit(1);
			}
			...
			if (readExcludesFromFile) {
				printf(
					"WARNING: More than one exclude file specified. Ignoring new exclude file.\n");
			} else {
			...
			if (readBondAnglesFromFile) {
				printf("WARNING: More than one bondangle file specified. Ignoring new bondangle "
					   "file.\n");
			} else {
				bondAngleFile = value;
			...
			if (readBondAnglesFromFile) {
				printf("WARNING: More than one product potential file specified. Ignoring new "
					   "file.\n");
			} else {
			...
			if (readDihedralsFromFile) {
				printf("WARNING: More than one dihedral file specified. Ignoring new dihedral "
					   "file.\n");
			} else {
				dihedralFile = value;
			...
			if (readVecanglesFromFile) {
				printf("WARNING: More than one vecangle file specified. Ignoring new vecangle "
					   "file.\n");
			} else {
				vecangleFile = value;
			...
			if (readRestraintsFromFile) {
				printf("WARNING: More than one restraint file specified. Ignoring new restraint "
					   "file.\n");
			} else {
				restraintFile = value;
			...
			stringToArray<float>(&value,
								 part[currPart].numPartGridFiles,
								 &partGridFileScale[currPart]);
		} else if (param == String("gridFileScaleSlope")) {
			if (currPart < 0)
				exit(1);
			// partGridFileScale[currPart] = (float) strtod(value.val(), NULL);
			stringToArray<float>(&value,
								 part[currPart].numPartGridFiles,
								 &partGridFileScaleSlope[currPart]);
		} else if (param == String("gridFileSMD")) {
			// One parameter for each grid file with inverse of number of steps for doubling
			// (positive) or half (negative) time of grid size; zero implies no change
			if (currPart < 0)
				exit(1);
			stringToArray<float>(&value,
								 part[currPart].numPartGridFiles,
								 &partGridFileSMD[currPart]);
			...
			if (num > 0) {
				...
				for (size_t i = 0; i < num; ++i) {
					...
					if (tokens[i] == "dirichlet")
						...
						fprintf(stderr,
								"WARNING: Unrecognized gridFile boundary condition \"%s\". Using "
								"Dirichlet.\n",
								tokens[i].val());
						data[i] = dirichlet;
					}
		...
		// set damping here, using anisotropic damping, i.e. data type Vector3 Han-Yi Chou
			...
			if (currPartClass == partClassPart) {
				...
				partGridFileScaleSlope[currPart] = new float[num];
				for (int i = 0; i < num; ++i) {
					// printf("%s ", partGridFile[currPart]->val());
					partGridFileScaleSlope[currPart][i] = 0.0f;
				}
				// Set default grid smd paramter
				partGridFileSMD[currPart] = new float[num];
				for (int i = 0; i < num; ++i) {
					partGridFileSMD[currPart][i] = 0.0f;
				}
...
void Configuration::Print() {
	...
	for (int i = 0; i < numParts; ++i) {
		...
		printf("The translational damping is %f %f %f \n",
			   part[i].transDamping.x,
			   part[i].transDamping.y,
			   part[i].transDamping.z);
	}
...
void Configuration::readAtoms() {
	...
	if (inp == NULL) {
		...
		for (int i = 0; i < numParts; i++)
			if (part[i].num == 0)
				found = false;
		// assert(false); // TODO probably relax constraint that particle must be found; could just
		// be in RB
		if (!found) {
...
void Configuration::readGroups() {
	...
	// Get and process all lines of input
	while (fgets(line, line_char, inp) !=
		   NULL) { // TODO: write a templated parser that can handle arbitrarily long lines
		// Lines in the particle file that begin with # are comments
		if (line[0] == '#')
			continue;
...
void Configuration::readBonds() {
	...
	while (fgets(line, 256, inp) != NULL) {
		...
		if (ind1 < 0 || ind1 >= num + num_rb_attached_particles + numGroupSites || ind2 < 0 ||
			ind2 >= num + num_rb_attached_particles + numGroupSites) {
			printf("ERROR: Bond file line '%s' includes invalid index\n", line);
			exit(1);
		}

		if (tokenList[0] == "SMDBOND") {
			if (numTokens != 7) {
				printf("WARNING: Invalid bond file line: %s\n", line);
				continue;
			}
			float r0 = (float)strtod(tokenList[4].val(), NULL);
			float k = (float)strtod(tokenList[5].val(), NULL);
			float v = (float)strtod(tokenList[6].val(), NULL);

			int2 ids = make_int2(ind1, ind2);
			float4 p = make_float4(r0, k, v, 0.0f);
			smd_bond_particles.push_back(ids);
			smd_bond_params.push_back(p);
...
void Configuration::addExclusion(int ind1, int ind2) {
	if (ind1 >= num + num_rb_attached_particles || ind2 >= num + num_rb_attached_particles) {
		printf("WARNING: Attempted to add an exclusion for an out-of-range particle index (%d or "
			   "%d >= %d).\n",
			   ind1,
			   ind2,
			   num + num_rb_attached_particles);
		return;
	}
...
void Configuration::readAngles() {
	...
	while (fgets(line, 256, inp)) {
		...
		if (ind1 >= num + num_rb_attached_particles + numGroupSites or
			...
			ind3 >= num + num_rb_attached_particles + numGroupSites)
			continue;
...
void Configuration::readDihedrals() {
	const size_t max_char_per_line = 256;
	...
	while (fgets(line, max_char_per_line, inp)) {
		if (line[0] == '#')
			continue;
		String s(line);
		int numTokens = s.tokenCount();
		...
		if (tokenList[0] == "SMDDIHEDRAL") {
			...
			float r0 = (float)strtod(tokenList[5].val(), NULL);
			float k = (float)strtod(tokenList[6].val(), NULL);
			float v = (float)strtod(tokenList[7].val(), NULL);
			r0 *= PI / 180;						// convert degrees to radian
			k *= (180.0f / PI) * (180.0f / PI); // convert kcal_mol/degrees**2 to radian**2
			v *= (PI / 180) * timestep;			// convert deg/ns to rad/timestep

			int4 ids = make_int4(ind1, ind2, ind3, ind4);
			float4 p = make_float4(r0, k, v, 0.0f);
			smd_dihedral_particles.push_back(ids);
			smd_dihedral_params.push_back(p);
		} else {
			if (numTokens != 6) {
				printf("WARNING: Invalid dihedral input line: %s\n", line);
				continue;
...
void Configuration::readBondAngles() {
	...
	while (fgets(line, 256, inp)) {
		...
		// BONDANGLE | INDEX1 | INDEX2 | INDEX3 | INDEX4 | ANGLE_FILENAME | BOND_FILENAME1 |
		// BOND_FILENAME2
		if (numTokens != 8) {
...
void Configuration::readProductPotentials() {
	...
	while (fgets(line, 256, inp)) {
		...
		// BONDANGLE | INDEX1 | INDEX2 | INDEX3 | [TYPE1] | POT_FILENAME1 | INDEX4 | INDEX5 |
		// [TYPE2] POT_FILENAME2 ...
		if (numTokens < 5) {
			...
			if (tokenList[i].val() == end || *end != '\0' || errno == ERANGE) {
				...
				if (n == "bond") {
					...
					if (simple_potential_ids.find(n_str) == simple_potential_ids.end()) {
						...
						if (s < 2 || s > 4) {
							printf("WARNING: Invalid product potential input line (indices of "
								   "potential %d == %d): %s\n",
								   i,
								   s,
								   line);
						...
						if (not type_specified)
							type = s == 2 ? BOND : s == 3 ? ANGLE : DIHEDRAL;
...
void Configuration::readRestraints() {
	...
	while (fgets(line, 256, inp)) {
		...
		if (id >= num + num_rb_attached_particles + numGroupSites)
			continue;
...
void Configuration::getDebugForce() {
	...
	if (tabulatedPotential) {
		if (fullLongRange)
			printf("(6) was specified by config file\n");
		else
			printf("(5) was specified by config file\n");
	} else {
		if (fullLongRange != 0)
			printf("(%d) was specified by config file\n", fullLongRange);
		else
			printf("(4) was specified by config file\n");
	}
...
// Before using this code, make sure the array type list and serial list are both already
// initialized
...
bool Configuration::compare::operator()(const ProductPotentialConf& lhs,
										const ProductPotentialConf& rhs) {
