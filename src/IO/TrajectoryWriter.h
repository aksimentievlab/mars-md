...
class TrajectoryWriter {
  public:
	...
	TrajectoryWriter(std::string_view filePrefix,
					 ...
					 float timestep,
					 int outputPeriod)
		: box_(box), numAtoms_(numAtoms), timestep_(timestep), outputPeriod_(outputPeriod),
		  format_(format) {

		if (numAtoms <= 0) {
			throw Exception(ExceptionType::ValueError,
							SourceLocation(),
							"TrajectoryWriter: Invalid number of atoms: %d",
							numAtoms);
		}
  ...
  private:
	...
	void appendPdb(std::span<const Vector3> positions, std::span<const std::string> names) {
	...
	void writePdbAtoms(FILE* fp,
		...
		for (size_t i = 0; i < positions.size(); ++i) {
			...
			std::fprintf(fp,
						 "ATOM  %5zu  %-4s MOL S%4zu    %8.3f%8.3f%8.3f  1.00  0.00      ION\n",
						 i + 1,
						 name.c_str(),
						 i + 1,
						 pos.x,
	...
	void
	appendTraj(std::span<const Vector3> positions, std::span<const std::string> names, float time) {
