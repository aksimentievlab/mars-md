...
NanoGridAdapter<> load_nanovdb_grid(
    ...
    try {
        ...
        return NanoGridAdapter<>::from_file(filename, grid_name, resource);
    } catch (const std::exception& e) {
        ARBD_Exception(ExceptionType::RuntimeError, 
                      "Failed to load NanoVDB grid from {}: {}", filename, e.what());
    }
}
 ...
 */
template<typename ValueT = float>
std::vector<NanoGridAdapter<>> load_nanovdb_grids(
    const std::string& filename,
    const Resource& resource = Resource{ResourceType::CPU, 0}) {
    
    std::vector<NanoGridAdapter<>> result;
    ...
    try {
        ...
        ARBD_Exception(ExceptionType::RuntimeError,
                      "Failed to load NanoVDB grids from {}: {}", filename, e.what());
    }
}
...
inline std::vector<std::string> query_nanovdb_grids(const std::string& filename) {
    try {
        ...
        ARBD_Exception(ExceptionType::RuntimeError,
                      "Failed to query NanoVDB file {}: {}", filename, e.what());
    }
}
