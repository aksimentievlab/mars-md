...
#define THREADGROUP_PTR(T) T*
 ...
 public:
 ...
 #ifdef __METAL_VERSION__
		 using TU = T; // Metal doesn't support std::common_type
 #else
	 ...
	 // Only enable for floating point types - simplified for Metal compatibility
	 ...
	 // Element-wise operations - simplified for Metal compatibility
	 ...
	 HOST DEVICE static constexpr T highest() {
		 ...
		 // Metal doesn't have numeric_limits, use hardcoded values
		 if (sizeof(T) == 4) return T(3.40282347e+38f); // FLT_MAX
		 if (sizeof(T) == 8) return T(1.7976931348623157e+308); // DBL_MAX
		 return T(1);
 #else
	 ...
	 // String and printing (host-only for Metal compatibility)
