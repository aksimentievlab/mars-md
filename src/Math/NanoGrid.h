...
class NanoGrid {
    ...
    void updateGridCount(nanovdb::GridData *d_data, uint32_t gridIndex, uint32_t gridCount, bool *d_dirty)
    {
    ...
    VectorT<GridHandle<BufferT>> splitGridHandles(const GridHandle<BufferT> &handle, const BufferT* other = nullptr, 
    Event stream = Event())
    {
    ...
    namespace {auto __dummy(){return nanovdb::GridHandle<nanovdb::cuda::DeviceBuffer>(std::move(nanovdb::cuda::DeviceBuffer()));}}
