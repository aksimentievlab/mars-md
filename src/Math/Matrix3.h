 ...
 *********************************************************************/
 ...
 public:
	 ...
	 // Column accessors
	 const Vector3& ex() const { return cols[0]; }
	 const Vector3& ey() const { return cols[1]; }
	 const Vector3& ez() const { return cols[2]; }
	 
	 Vector3& ex() { return cols[0]; }
	 Vector3& ey() { return cols[1]; }
	 Vector3& ez() { return cols[2]; }
 
 #if !defined(__METAL_VERSION__) && !defined(__SYCL_DEVICE_ONLY__) && !defined(__CUDA_ARCH__)
	 // String representation - only available on host
	 auto to_string() const {
		 Matrix3 transposed = transpose();
		 return string_format("%2.8f %2.8f %2.8f\n%2.8f %2.8f %2.8f\n%2.8f %2.8f %2.8f",
							transposed.cols[0].x, transposed.cols[1].x, transposed.cols[2].x,
							transposed.cols[0].y, transposed.cols[1].y, transposed.cols[2].y,
							transposed.cols[0].z, transposed.cols[1].z, transposed.cols[2].z);
	 }
 #endif
 };
