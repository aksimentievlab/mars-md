...
public:
     ...
     */
    CellListPairlist(const Resource& resource, size_t max_particles, size_t max_pairs, int num_replicas = 1);
     ...
     * @param num_particles Number of particles
     */
    void update_pairlist(const DeviceBuffer<Vector3>& positions,
     ...
     */
    PairlistType get_type() const override { return PairlistType::CellList; }

    /**
     * @brief Get implementation name
     */
    const char* get_name() const override { return "Cell List Pairlist"; }
     ...
     */
    Vector3_t<int> get_cell_dimensions() const { return n_cells_; }
     ...
     */
    void set_num_replicas(int num_replicas) { num_replicas_ = num_replicas; }
