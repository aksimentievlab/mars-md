...
CellListPairlist::CellListPairlist(const Resource& resource, size_t max_particles, size_t max_pairs, int num_replicas)
    : Pairlist(resource, max_particles, max_pairs),
      ...
      cell_size_(1.0f),
      total_cells_(1),
      cells_(max_particles * num_replicas, resource),
      cell_ranges_(max_particles, resource),
      temp_ranges_(max_particles, resource),
      last_build_time_ms_(0.0),
      cells_used_(0),
      average_particles_per_cell_(0.0f) {

    LOGINFO("Created CellListPairlist with capacity {} particles, {} pairs, {} replicas on {}",
            max_particles_, max_pairs_, num_replicas_, resource_.toString());
}
...
void CellListPairlist::update_pairlist(const DeviceBuffer<Vector3>& positions, size_t num_particles) {
...
Pairlist::Statistics CellListPairlist::get_statistics() const {
    ...
    stats.cells_used = cells_used_;
    stats.average_particles_per_cell = average_particles_per_cell_;
    return stats;
}
...
void CellListPairlist::compute_cell_grid(float cutoff) {
    ...
    LOGTRACE("Cell grid: {}x{}x{} = {} cells, cell size = {:.3f}",
             n_cells_.x, n_cells_.y, n_cells_.z, total_cells_, cell_size_);
}

void CellListPairlist::decompose_particles(const DeviceBuffer<Vector3>& positions, size_t num_particles) {
...
void CellListPairlist::sort_particles_by_cell() {
    ...
    thrust::sort(cells_ptr, cells_ptr + num_particles_ * num_replicas_,
...
void CellListPairlist::create_cell_ranges() {
    ...
    MakeRangesKernel make_kernel{
        static_cast<int>(total_cells_),
        num_replicas_
    };

    size_t total_elements = num_particles_ * num_replicas_;
    KernelConfig config1 = KernelConfig::for_1d(total_elements, resource_);
    launch_kernel(resource_, config1, make_kernel, cells_.data(), temp_ranges_.data(), total_elements);

    // Step 2: Bind ranges from temporary array
    BindRangesKernel bind_kernel{
        static_cast<int>(total_cells_),
        num_replicas_
    };

    KernelConfig config2 = KernelConfig::for_1d(total_cells_ * num_replicas_, resource_);
    launch_kernel(resource_, config2, bind_kernel, cell_ranges_.data(), temp_ranges_.data(), cells_.data(), total_elements);
}

void CellListPairlist::find_neighbors_celllist(const DeviceBuffer<Vector3>& positions, size_t num_particles) {
    CellNeighborKernel kernel{
        positions.data(),
        cells_.data(),
        cell_ranges_.data(),
