...
class CutoffAMRElectrostatics : public LongRangeElectrostatics {
  private:
	...
	std::vector<std::unique_ptr<BaseGrid<mars_real>>> amr_levels_;
...
class PPPMElectrostatics : public LongRangeElectrostatics {
  private:
	...
	std::unique_ptr<BaseGrid<mars_real>> potential_grid_;
  ...
  private:
	void initialize_grids() {
		...
		charge_grid_ = std::make_unique<BaseGrid<mars_real>>(basis, origin, nx, ny, nz);
		potential_grid_ = std::make_unique<BaseGrid<mars_real>>(basis, origin, nx, ny, nz);
