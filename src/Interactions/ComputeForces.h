...
constexpr size_t MAX_NLIST_PAIRS = 1 << 27;
// Reduce if ARBD crashes immediately with GPU memory allocation error
...
HOST DEVICE coulombForce(Vector3 r, float alpha, float start, float len) {
	float d = r.length();
...
HOST DEVICE EnergyForce coulombForceFull(Vector3 r, float alpha) {
	float d = r.length();
	return EnergyForce(alpha / d, -alpha / (d * d * d) * r);
}

HOST DEVICE EnergyForce softcoreForce(Vector3 r, float eps, float rad6) {
	...
	void computeFullKernel(Vector3 force[],
						   Vector3 pos[],
						   int type[],
						   float tableAlpha[],
						   float tableEps[],
	...
	void computeSoftcoreFullKernel(Vector3 force[],
								   Vector3 pos[],
								   int type[],
								   float tableEps[],
								   float tableRad6[],
	...
	__global__ void computeElecFullKernel(Vector3 force[],
										  Vector3 pos[],
										  int type[],
										  float tableAlpha[],
										  int num,
	...
	//
	DEVICE int computeCellNeighbor(const int3 cells,
								   const int3 cell_idx,
								   const int dx,
								   const int dy,
								   const int dz) {
	...
	void createPairlists(Vector3* __restrict__ pos,
						 const int num,
				...
				for (int pid_i = pid_start; pid_i < Ni; pid_i += Step1) {
					...
					if (idx_ + pid_i < Ni) {
						...
						for (int idx = 0; idx < 27; ++idx) {
							...
							CellDecomposition::range_t rangeJ =
								decomp->getRange(neighbor_cell, repID);
							...
							for (int pid_j = idx__; pid_j < Nj; pid_j += Step2) {
								...
								while (nextEx >= 0 && nextEx < (aj - repID * num)) {
									nextEx =
										(currEx < ex_pair.y - 1) ? excludes[++currEx].ind2 : -1;
								}
	...
	void createPairlists_debug(Vector3* __restrict__ pos,
							   const int num,
							   const int numReplicas,
							   const BaseGrid* __restrict__ sys,
							   const CellDecomposition* __restrict__ decomp,
		...
		for (int cID = 0 + (blockIdx.x % split); cID < nCells; cID += split) {
			for (int repID = 0; repID < numReplicas; repID++) {
				...
				for (int ci = rangeI.first + blockIdx.x / split; ci < rangeI.last;
					...
					const int ex_start = (numExcludes > 0 && excludeMap != NULL)
											 ? excludeMap[ai - repID * num].x
											 : -1;
					const int ex_end = (numExcludes > 0 && excludeMap != NULL)
										   ? excludeMap[ai - repID * num].y
										   : -1;
					for (int x = -1; x <= 1; ++x) {
						for (int y = -1; y <= 1; ++y) {
							for (int z = -1; z <= 1; ++z) {
								...
								const CellDecomposition::range_t range =
									decomp->getRange(nID, repID);
								for (int n = range.first + tid; n < range.last; n += blockDim.x) {
									...
									// assert( ajLast < aj ); ajLast = aj; // TODO: remove this
									// sanity check
									while (nextEx >= 0 &&
										   nextEx < (aj - repID * num)) // TODO get rid of this
										nextEx =
											(currEx < ex_end - 1) ? excludes[++currEx].ind2 : -1;
									if (nextEx == (aj - repID * num)) {
#ifdef DEBUGEXCLUSIONS
										atomicAggInc(exSum, warpLane);
#endif
	...
	// TODO: deprecate?
	void computeKernel(Vector3 force[],
					   Vector3 pos[],
					   int type[],
					   float tableAlpha[],
					   float tableEps[],
	...
	DEVICE int pairForceCounter = 0;
	void printPairForceCounter() {
		if (threadIdx.x + blockIdx.x == 0)
			printf("Computed the force for %d pairs\n", pairForceCounter);
	}

	template<const int BlockSize>
	DEVICE inline void _computeTabulatedKernel(Vector3 * force,
											   const BaseGrid* __restrict__ sys,
											   float cutoff2,
											   const int numPairs,
	...
	__global__ void computeTabulatedEnergyKernel(Vector3 * force,
		...
		for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < numPairs;
	...
	//
	__global__ void computeTabulatedFullKernel(Vector3 force[],
											   Vector3 pos[],
											   int type[],
		...
		if (i < num * numReplicas) {
			...
			for (int j = first; j < last; ++j) {
				...
				if (nextBond == (j - repID * num) and tableBond != NULL) {
					...
					if (currBond < bond_end - 1)
						nextBond = bonds[++currBond].ind2;
					else
						nextBond = -1;
				}
	...
	__global__ void computeAngles(Vector3 force[],
								  Vector3 pos[],
								  Angle angles[],
	...
	__global__ void computeTabulatedBonds(Vector3 * force,
										  ...
										  BaseGrid* __restrict__ sys,
										  int numBonds,
										  int3* __restrict__ bondList_d,
										  TabulatedPotential** tableBond,
										  float* energy,
										  bool get_energy) {
		// Loop over ALL bonds in ALL replicas
		for (int bid = threadIdx.x + blockIdx.x * blockDim.x; bid < numBonds;
	...
	__global__ void computeTabulatedAngles(Vector3 * force,
										   Vector3* __restrict__ pos,
										   BaseGrid* __restrict__ sys,
										   int numAngles,
										   int4* __restrict__ angleList_d,
										   TabulatedAnglePotential** tableAngle,
										   float* energy,
										   bool get_energy) {
		// Loop over ALL angles in ALL replicas
		for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < numAngles;
			 i += blockDim.x * gridDim.x) {
			int4& ids = angleList_d[i];
			computeAngle(tableAngle[ids.w],
						 sys,
						 force,
						 pos,
						 ids.x,
	...
	__global__ void computeTabulatedBondAngles(Vector3 * force,
											   Vector3* __restrict__ pos,
											   BaseGrid* __restrict__ sys,
											   int numBondAngles,
											   int4* __restrict__ bondAngleList_d,
											   TabulatedAnglePotential** tableAngle,
											   TabulatedPotential** tableBond,
											   float* energy,
											   bool get_energy) {
		// Loop over ALL angles in ALL replicas
		for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < numBondAngles;
			 i += blockDim.x * gridDim.x) {
			...
			computeBondAngle(tableAngle[angleInd1],
							 tableBond[bondInd],
							 tableAngle[angleInd2],
							 sys,
							 force,
							 pos,
							 atom1,
	...
	__global__ void computeProductPotentials(Vector3 * force,
											 Vector3* __restrict__ pos,
											 BaseGrid* __restrict__ sys,
											 ...
											 bool get_energy) {
		/*
		  productPotential_list[i].x : index of first potential in potentialList for i_th
		productPotential productPotential_list[i].y : index of first atom in
		productPotentialParticles for i_th productPotential

		  for three potentials, angle, bond, angle, we would have the following atomic indices in
		productPotentialParticles: pot1 : productPotential_list[i].y, productPotential_list[i].y + 1
		, productPotential_list[i].y + 2 pot2 : productPotential_list[i].y + 3,
		productPotential_list[i].y
		+ 4 and pot3 : productPotential_list[i].y + 5, productPotential_list[i].y + 6,
		productPotential_list[i].y + 7
		...
		*/
		...
		for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < numProductPotentials;
			 i += blockDim.x * gridDim.x) {
...
#pragma unroll
			for (unsigned short int j = 0; j < MAX_XPOTS; ++j) {
				...
				// Hidden branch divergence in compute_value => sort potentials by type before
				// running kernel
...
#pragma unroll
	...
	__global__ void computeDihedrals(Vector3 force[],
									 Vector3 pos[],
									 Dihedral dihedrals[],
	...
	// void computeTabulatedDihedrals(Vector3* __restrict__ force, Vector3* __restrict__ pos, int
	// num, 			    int numParts, BaseGrid* __restrict__ sys, int4* __restrict__
	// dihedralList_d, 			    int*
	// __restrict__ dihedralPotList_d, 			    int numDihedrals, int numReplicas, float*
	// __restrict g_energies, 			    bool get_energy, TabulatedDihedralPotential**
	// __restrict__ tableDihedral) {

	__global__ void computeTabulatedDihedrals(Vector3 * force,
											  const Vector3* __restrict__ pos,
											  const BaseGrid* __restrict__ sys,
											  int numDihedrals,
											  const int4* const __restrict__ dihedralList_d,
											  const int* __restrict__ dihedralPotList_d,
											  TabulatedDihedralPotential** tableDihedral,
											  float* energy,
											  bool get_energy) {
		...
		for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < numDihedrals;
			 i += blockDim.x * gridDim.x) {
			const int4& ids = dihedralList_d[i];
			const int& id = dihedralPotList_d[i];
			computeDihedral(tableDihedral[id],
							sys,
							force,
							pos,
							ids.x,
	...
	__global__ void computeHarmonicRestraints(Vector3 * force,
											  const Vector3* __restrict__ pos,
											  const BaseGrid* __restrict__ sys,
											  int numRestraints,
											  const int2* const __restrict__ restraintList,
											  const Vector3* __restrict__ r0,
											  const float* __restrict__ k) {

		// Loop over ALL dihedrals in ALL replicas
		for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < numRestraints;
			 i += blockDim.x * gridDim.x) {
