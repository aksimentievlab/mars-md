...
class TabulatedAnglePotential {
  public:
	...
	TabulatedAnglePotential(std::string fileName);
	TabulatedAnglePotential(const TabulatedAnglePotential& tab);
	~TabulatedAnglePotential();
	float* pot;			  // actual potential values
	float angle_step_inv; // '1/step' angle in potential file. potential file might not go 1, 2,
						  // 3,...,360, it could be in steps of .5 or something smaller
	int size;			  // The number of data points in the file
	std::string fileName;

/*
	HOST DEVICE inline EnergyForce
	computeOLD(Angle* a, Vector3* pos, BaseGrid<float>* sys, int index) {
		...
		const Vector3 ab = posa - posb;
		const Vector3 bc = posb - posc;
		const Vector3 ac = posc - posa;
