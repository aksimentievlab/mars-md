...
DEVICE inline void computeDihedral(const BondedPotential* __restrict__ d,
	...
	// posb.x,posb.y,posb.z, 		       posc.x,posc.y,posc.z, posd.x,posd.y,posd.z);
	...
	// force = (distbc*distbc*crossABC.rLength2() > 1000.0f || distbc*distbc*crossBCD.rLength2() >
	// 1000.0f) ? 0.0f : force;
	force = (ab.length2() * bc.length2() * crossABC.rLength2() > 100.0f ||
			 bc.length2() * cd.length2() * crossBCD.rLength2() > 100.0f)
				? 0.0f
				: force;

	// if ( force > 1000.0f )
	//     printf("%f %d %d (%.4f %.4f) %.2f %f\n",force,home,home1, d->pot[home], d->pot[home1],
	//     dU, d->angle_step_inv);
