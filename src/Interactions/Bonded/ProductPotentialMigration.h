...
struct LegacyBondAngle {
	...
	std::string angle_file1;
	std::string bond_file;
	std::string angle_file2;
	int tab_file_index1;
	int tab_file_index2;
	int tab_file_index3;

	LegacyBondAngle()
		: ind1(-1), ind2(-1), ind3(-1), ind4(-1), tab_file_index1(-1), tab_file_index2(-1),
		  tab_file_index3(-1) {}
	...
	LegacyBondAngle(int i1,
					...
					const std::string& b,
					const std::string& a2)
		: ind1(i1), ind2(i2), ind3(i3), ind4(i4), angle_file1(a1), bond_file(b), angle_file2(a2),
		  tab_file_index1(-1), tab_file_index2(-1), tab_file_index3(-1) {}
};
...
migrate_bond_angle_to_product_potential(const LegacyBondAngle& legacy_bond_angle) {
	return ProductPotential::from_bond_angle(
		legacy_bond_angle.ind1,
		legacy_bond_angle.ind2,
		legacy_bond_angle.ind3,
		legacy_bond_angle.ind4,
		legacy_bond_angle.tab_file_index1, // angle 1 table index
		legacy_bond_angle.tab_file_index2, // bond table index
		legacy_bond_angle.tab_file_index3, // angle 2 table index
		"MigratedBondAngle");
}
