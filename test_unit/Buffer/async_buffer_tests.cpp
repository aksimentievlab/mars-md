	...
	AsyncBufferTestFixture() {
		...
		try {
			...
			if (SYCL::Manager::devices().size() > 0) {
				...
				std::cout << "Added " << SYCL::Manager::devices().size() << " SYCL device(s) to available resources" << std::endl;
			}
#endif
#ifdef USE_METAL
			METAL::Manager::init();
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("PinnedBuffer with async streams") {
		...
		if (stream == nullptr) {
			SKIP("PinnedBuffer stream not available on this platform - AdaptiveCpp limitation");
		}
		REQUIRE(stream != nullptr);
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("UnifiedBuffer memory advice") {
		...
		if (device.type == ResourceType::CUDA) {
			REQUIRE_NOTHROW(buffer.mem_advise(cudaMemAdviseSetPreferredLocation, static_cast<int>(device.id)));
			REQUIRE_NOTHROW(buffer.mem_advise(cudaMemAdviseSetAccessedBy, static_cast<int>(device.id)));
		}
#endif
	...
	SECTION("UnifiedBuffer expandable operations") {
		...
		// REQUIRE_NOTHROW(buffer.prefetch_range(range_offset, range_size, static_cast<int>(device.id)));
		// REQUIRE_NOTHROW(buffer.advise_range(range_offset, range_size, static_cast<int>(device.id), 0));
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("Mixed buffer type async operations") {
		// Test async operations between different buffer types
		DeviceBuffer<double> device_buffer(buffer_size, device);
		PinnedBuffer<double> pinned_buffer(buffer_size, device);
		
		// Initialize device buffer
		std::vector<double> test_data(buffer_size, 1.23456);
		device_buffer.copy_from_host_sync(test_data.data(), buffer_size);
		
		// Copy from device buffer to pinned buffer (device-to-device copy)
		pinned_buffer.copy_device_to_device(pinned_buffer.data(), device_buffer.data(), buffer_size);
		
		// Verify data in pinned buffer
		std::vector<double> result_data(buffer_size);
		pinned_buffer.copy_to_host(result_data.data(), buffer_size);
		REQUIRE(result_data == test_data);
	}
}
...
TEST_CASE_METHOD(AsyncBufferTestFixture,
	...
	SECTION("Async operations with oversized transfers") {
		...
		REQUIRE_THROWS_AS(buffer.copy_from_host_async(oversized_data.data(), oversized_data.size()), Exception);
	}
