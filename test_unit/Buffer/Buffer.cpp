...
TEST_CASE_METHOD(ProductionBufferTestFixture, 
    ...
    SECTION("Concurrent buffer creation") {
        const size_t num_threads = std::min(static_cast<size_t>(8), std::max(devices.size(), static_cast<size_t>(1)));
        ...
        for (size_t i = 0; i < num_threads; ++i) {
            futures.emplace_back(std::async(std::launch::async, [i, buffer_size, &devices]() {
                try {
                    ...
                    Resource resource = devices.empty() ? 
                        Resource::CPU() : 
                        devices[i % devices.size()];
                    
                    std::vector<std::unique_ptr<DeviceBuffer<float>>> buffers;
                    for (int j = 0; j < 5; ++j) {
                        auto buffer = std::make_unique<DeviceBuffer<float>>(buffer_size + j, resource);
    ...
    SECTION("Concurrent memory operations") {
        ...
        for (size_t i = 0; i < std::min(devices.size(), static_cast<size_t>(4)); ++i) {
            buffers.emplace_back(
                std::make_unique<DeviceBuffer<int>>(buffer_size, devices[i % devices.size()]));
        }
...
#ifdef USE_CUDA
TEST_CASE_METHOD(ProductionBufferTestFixture, 
                 "CUDA-Specific Buffer Operations", 
                 "[buffer][cuda]") {
...
TEST_CASE_METHOD(ProductionBufferTestFixture, 
    ...
    SECTION("Concurrent memory operations") {
        ...
        for (size_t i = 0; i < devices.size(); ++i) {
            buffers.emplace_back(
                std::make_unique<DeviceBuffer<int>>(buffer_size, devices[i]));
        }
    ...
    SECTION("Multi-GPU workload distribution") {
        ...
        for (size_t i = 0; i < devices.size(); ++i) {
            ...
            REQUIRE(std::all_of(result.begin(), result.end(), 
                               [i](float val) { return val == static_cast<float>(i); }));
        }
    }
}
...
TEST_CASE_METHOD(ProductionBufferTestFixture, 
    ...
    SECTION("Production environment simulation") {
        ...
        const size_t num_workers = std::min(static_cast<size_t>(4), 
                                           std::max(devices.size(), static_cast<size_t>(1)));
        ...
        for (size_t worker_id = 0; worker_id < num_workers; ++worker_id) {
            worker_futures.emplace_back(std::async(std::launch::async, 
                [worker_id, work_items, item_size, &devices]() -> bool {
                try {
                    Resource worker_device = devices.empty() ? 
                        Resource::CPU() : 
                        devices[worker_id % devices.size()];
