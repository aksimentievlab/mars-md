...
#include "System/NanoVDBNeighbors.h"
...
TEST_CASE("NanoVDB Gradient Computation", "[nanovdb][stencil][gradient]") {
    ...
    SECTION("Compute gradients for sample coordinates") {
        std::vector<nanovdb::Coord> sample_coords = {
        ...
        DeviceBuffer<nanovdb::math::Vec3<float>> gradient_results(sample_coords.size(), cuda_resource);
        ...
        auto gradient_event = nano_processor.compute_gradients(
            coord_buffer, gradient_results, grid, config);
        gradient_event.wait();
...
TEST_CASE("NanoVDB Laplacian Computation", "[nanovdb][stencil][laplacian]") {
    ...
    SECTION("Compute Laplacians for sample coordinates") {
        ...
        auto laplacian_event = nano_processor.compute_laplacians(
            coord_buffer, laplacian_results, grid, config);
...
TEST_CASE("NanoVDB Interpolation", "[nanovdb][interpolation]") {
    ...
    SECTION("Interpolate values at world coordinates") {
        std::vector<nanovdb::math::Vec3<float>> world_positions = {
            {1.5f, 2.5f, 3.5f}, {4.5f, 5.5f, 6.5f}, {7.5f, 8.5f, 9.5f}
        };
        
        DeviceBuffer<nanovdb::math::Vec3<float>> position_buffer(world_positions.size(), cuda_resource);
        ...
        auto interp_event = nano_processor.interpolate_values(
            position_buffer, interpolation_results, grid, config);
        interp_event.wait();
